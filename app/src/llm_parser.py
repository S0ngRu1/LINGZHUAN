...
def _parse_single_chunk(client: OpenAI, chunk_content: str, chunk_num: int, total_chunks: int) -> Optional[List[Dict]]:
    ...
    """
    prompt = f"""
            你是一个专业的项目管理助理。你的任务是从以下文本中提取所有的项目任务安排，并严格按照指定的JSON格式输出。
            ...
            3. 每个对象必须包含且仅包含以下六个键："阶段", "工作子项", "成果文件", "主责单位/负责人", "开始日期", "结束日期"。
            4. "开始日期" 和 "结束日期" 的值必须是 "2025-MM-DD" 格式的字符串。如果只提到一个日期，则开始和结束日期相同。
            5. 如果文本中没有明确提到某个任务的 "成果文件" 或 "主责单位/负责人"，请将对应的值设为 `null`。
            6. 如果文本中没有可提取的任何任务，请返回一个空数组 `[]`。
            7. 不要包含任何JSON格式之外的解释、注释或文字。只输出纯净的JSON。
        """
    try:
