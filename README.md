...
## 功能扩展
### 添加功能：自己上传多个文件并进行对比分析

1. 使用现有的文件读取代码`app/utils/file_handler.py`,它可以读取上传的文件中的内容
2. 使用现有的大模型分析代码`app/src/llm_analyze_document.py`,它可以的对上一步得到的内容进行一个解析。
   - 它包括两个功能:
        - 对每个文件的内容提取摘要和大纲:函数`analyze_single_document`。
        - 对两个文件的摘要进行对比:函数`compare_summaries`。
   把这两个功能串起来，就能实现对自己上传多个文件进行对比分析
3. 最后要在`server.py`中添加该服务
   - 我们定义一个接口，把它命名为：`'/api/llm-analyze-document-v2'`
   - 把步骤二的功能写在这个接口中，就可以实现功能的扩展了